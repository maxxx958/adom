...
while True:
    ...
    if not ret:
        ...
        # Extract the convex hull for the frame
        for idx, filter in enumerate(filters):
            ...
            if filter['morph']:
                ...
                for i in range(len(hull2)):
                    cv2.line(convex_hull_frame, hull2[i], hull2[(i + 1) % len(hull2)], (0, 255, 0), 2)

                convex_hull_frames.append(cv2.cvtColor(convex_hull_frame, cv2.COLOR_BGR2RGB))
...
imageio.mimsave('convex_hull_output.gif', convex_hull_frames, fps=20, loop=0)

print("GIFs saved as 'output.gif' and 'convex_hull_output.gif'")
